...
export function OutcomeLine<T>({ taken, noun = 'records', singular, emptyText }: { taken: Taken<T>; noun?: string; singular?: string; emptyText?: string }) {
  ...
  if (taken.state === 'lost' && !r) {
    return <p className={styles.line}><Glyph kind="warn" /> The dashboard could not get this reading: {taken.problem}. <button className={styles.action} onClick={taken.retake}>Try again</button></p>;
  }
  if (!r) {
    return <p className={styles.line}>{taken.state === 'taking' ? 'Taking the reading…' : ''}</p>;
  }
  ...
  return (
    <div className={styles.block}>
      ...
      {taken.held && taken.heldParams?.count !== undefined && taken.heldParams.count !== taken.requestedParams?.count ? (
        <p className={styles.line}>Showing last {taken.heldParams.count} while asking for last {taken.requestedParams?.count}. The visible rows belong to the earlier reading.</p>
      ) : null}
      {taken.held && taken.state === 'lost' ? <p className={styles.line} role="status"><Glyph kind="warn" /> Latest take could not reach Sentinel: {taken.problem}. The held reading remains visible.</p> : null}
      {taken.held && taken.latestFailure ? <p className={styles.line} role="status"><Glyph kind="warn" /> Latest take was not observed: {NOT_OBSERVED[taken.latestFailure.outcome] ?? taken.latestFailure.outcome}{taken.latestFailure.error?.detail ? ` · ${firstLine(taken.latestFailure.error.detail)}` : ''}. The held reading remains visible.</p> : null}
      {r.warnings.length ? (
...
export function Glyph({ kind }: { kind: 'critical' | 'error' | 'warning' | 'info' | 'warn' }) {
  ...
  return (
    <svg className={styles.glyph} data-kind={k} viewBox="0 0 10 10" aria-hidden="true">
      ...
      {k === 'info' && <path d="M2.2 5h5.6" strokeWidth="1.4" />}
    </svg>
  );
}
