...
| `dumps` | The crash-dump inventory under the Windows dump locations | `files` (raw: name, path, bytes, modified), `collection` (raw: per-location outcomes and gaps) | |
| `dump_header` | A bounded structural read of one inventoried dump: its exact header bytes and, according to format, a kernel bug check or a stream minidump's directory and fixed exception and system metadata | `file` (raw metadata), `header` (raw bytes, values and offsets), `streams` (raw MDMP directory entries and sampled metadata bytes, when present), `inspection` (derived: format, bug check or exception, stream counts, and limits), `collection` (raw inventory coverage) | `path` (required; exact path from `dumps` or `crash`) |
