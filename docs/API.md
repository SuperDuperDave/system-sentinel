...
`storms` reports the System WHEA-Logger query, its exact one-extra-record truncation result and the log's enabled/mode/oldest evidence in `collection`. The Windows clock supplies both query bounds and bucket alignment, so WSL clock skew cannot leave returned records outside the buckets. `coverage.system` derives the same retained-history boundary as `changes`. In `buckets.totals`, `0` means that bucket was observed without a matching event; `null` means coverage of the whole bucket could not be established from retention and the response cap. `buckets.total` and `active` count returned events, including those inside an incomplete bucket. A burst can be established from returned events even when surrounding time is unknown. Acceleration needs an observed recent and baseline window; quiet needs the whole requested window covered. Otherwise `status.state` is `unknown` and its rates are null where they cannot be calculated. The final bucket is only observed through `collection.window_end`, a millisecond-aligned host query time. A clean empty event query can therefore coexist with an unknown storm status. The Hardware errors trace shades buckets of unknown coverage and keeps returned event stretches available to inspect.
