...
| `dump_header` | A bounded read of one file from `dumps`, or a stop's matched dump: its header format and, for a recognized x64 Windows kernel header, the bug check and four parameters recorded in the file | `file` (raw metadata), `header` (raw fields extracted from at most 96 bytes), `inspection` (derived: format, architecture, bug check name where known, and the limit of the read) | `path` (required; exact path from `dumps` or `crash`) |
...
`dump_header` opens only a file already found in the Windows dump inventory. It reads at most the first 96 bytes. `PAGE`/`DU64` is interpreted as the documented 64-bit Windows kernel dump header; another signature is reported without guessed crash fields. The exact hex values in `header` come from the file, and `inspection` names the known bug check, if any. `empty` means the selected path is no longer inventoried. `denied` means Windows listed it but refused to open it; an ordinary process may encounter that in `C:\Windows\Minidump`. A recognized header does **not** prove the rest of the file is complete or valid, identify a failing driver, unwind a stack, or diagnose a cause. Full analysis needs a debugger and often symbols; no debugger is launched and no symbols are fetched by this reading. Microsoft documents the [DUMP_HEADER64 fields](https://microsoft.github.io/windows-docs-rs/doc/windows/Win32/System/Diagnostics/Debug/struct.DUMP_HEADER64.html) and [DumpChk's fuller validation](https://learn.microsoft.com/en-us/windows-hardware/drivers/debugger/dumpchk).
