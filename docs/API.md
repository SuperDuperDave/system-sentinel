...
```json
{
  ...
  "took_ms": 812,
  "outcome": "ok",
  "method": { "kind": "powershell", "query": "...Get-WinEvent -FilterXml $xml -MaxEvents 51..." },
  "count": 50,
  "sections": [
    { "name": "records", "class": "raw", "data": [ ... ] },
    { "name": "collection", "class": "raw", "data": { "limit": 50, "returned": 50, "truncated": true } }
  ],
  "error": null,
  "warnings": [],
  "redacted": ["host", "user"]
...
For `events`, `record`, `whea` and `faults`, `collection` says whether matching records exist past the requested `count`. The query asks Windows for one extra record, which is used only to set `truncated` and is never returned in `records` or decoded evidence. `limit` is the requested count and `returned` is the number delivered. An observed empty result has `{ "returned": 0, "truncated": false }`; a failed or unavailable query has no `collection` section. A `since` window on `events` or `faults` warns when it was cut short. A plain newest-N or before-a-moment request has no cutoff warning, but agents can inspect the flag to decide whether to ask again. Numeric minimum and maximum values in the catalog and MCP tool schemas are enforced before PowerShell runs. In JSON tool calls, explicit `null` for an optional parameter uses its published default; send `levels: []` to request every event level. In URL queries, omit a parameter to use its default.
...
| `events` | Records from a Windows log by level, and from a moment or this session's start | `records` (raw), `collection` (raw cutoff) | `log` (System, Application), `levels` (1 critical, 2 error, 3 warning, 4 information; default 1,2), `count` (default 50, 1 to 2000), `since` (an ISO timestamp or the word `boot`; empty for the most recent records) |
| `record` | The log around a moment: the records before a timestamp, oldest first | `records` (raw), `collection` (raw cutoff) | `before` (ISO timestamp, required: `422` without it), `count` (default 50, 1 to 2000), `log` (default System) |
...
| `faults` | What went wrong while the machine kept running: the programs that crashed or hung, and the kernel's own live reports (a GPU timeout, a watchdog) | `records` (raw, with `Log`), `collection` (raw cutoff), `decoded` (derived: the application, module, exception and supported process facts with per-field validity; a live kernel report is one entry however many records it was written across), `summary` (derived: by kind, by application, by live kernel report) | `count` (default 30, 1 to 500), `since` (an ISO timestamp or the word `boot`) |
| `whea` | WHEA-Logger records with their binary payload, each decoded beside it | `records` (raw, with `RawData` hex), `collection` (raw cutoff), `decoded` (derived: one entry per record, the decoder's structure or its error). The fixed CPER header and section bounds are checked before the external decoder runs. A malformed or unreadable record has a per-entry error; a missing decoder also adds a reading warning. The outcome stays the log's, because the records were observed either way | `count` (default 30, 1 to 500) |
| `storms` | WHEA records over a window in wall-clock buckets, grouped by signature, with burst and acceleration flags | `buckets` (derived: counts in covered buckets, null where coverage is unknown), `signatures` (derived), `status` (inferred: quiet, burst, accelerating or unknown, with the rates and the reason), `collection` (raw query and log metadata), `coverage` (derived retention reach) | `hours` (default 24, 1 to 43800), `bucket_seconds` (default 60, at least 1), `burst_threshold` (default 5, at least 1), `accel_threshold` (2.0, greater than 0) |
...
| `verbosity` | `summary` (a compact record table; for logs over 100 rows, first and last five rows, leading sources and cutoff; for `changes`, interpreted entries with source outcomes and coverage; for `storms`, status, coverage, collection, bounded timeline highlights and returned signature counts; or signal identity, title, summary and source readings) or `full` (selected records or signals with evidence as JSON; a `changes` selection puts its interpreted entries before exact raw rows). Defaults to `summary` for a whole `storms` reading or a whole `events`, `record`, `whea` or `faults` reading over 100 rows, and `full` otherwise; either choice can be changed in the Stack. |
