...
**The short prompt.** Run through a one-shot Claude Code agent (Sonnet) on 2026-09-21 on this machine, where the tool was already installed and running: the agent read the repository, took the skip in step 1, kept the token inline and never printed it, saw the catalog and the 401, registered the MCP server and confirmed it connected (the registration was removed afterwards, as the prompt says it can be), and read `events` through it. Its download steps ran on the clean image by the same harness against a local copy of the release files (`-Release` with a `file://` address), because the release was not yet published when the run was made: the download, the SHA-256 checked against `SHA256SUMS.txt`, `Unblock-File`, the start, the first answer within ten seconds, `health` and `events` ok. The download from GitHub itself was observed once the release was published, the same day: the harness ran the same steps against the published address, and the download, the checksum against the published `SHA256SUMS.txt`, `Unblock-File`, the start, the first answer within ten seconds and the `health` and `events` readings all passed, the whole run in under a minute.
