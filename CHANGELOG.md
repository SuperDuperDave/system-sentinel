...
- The MCP surface is the whole protocol rather than a tool list. Every tool is annotated with what it does to the machine, so a client can stop confirming twenty-two harmless readings and keep confirming the two that remove things. Every reading answers with typed structured content beside its text against one shared schema, so an agent branches on `outcome` as a field instead of parsing a string. The six presets are MCP prompts; the catalog and the composed handoff are resources; captures are tools. Asking for an unredacted view now requires a reason, and the reason is carried into the answer's warnings.
