...
- `dump_header` opens only a selected file from the Windows dump inventory and reads at most 96 bytes through the bridge. A recognized x64 kernel header yields the stop code and four parameters from the file, with raw fields separate from the interpretation; it never claims to validate the rest of the dump or identify a cause.
