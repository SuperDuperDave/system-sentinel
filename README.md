...
| `whea`, `whea_record`, `storms`, `whea_reports` | Hardware-error records from both Windows WHEA logs, with CPER header facts. `whea` gives the newest 500; `whea_record` retrieves one retained event by its source and log-local RecordId, including older reports. Exact binary bytes require an explicit unredacted request; default responses withhold them. Detailed decoding is currently limited to System WHEA-Logger records. `storms` counts that System source over live or historical filing-time buckets with coverage and fixed-header PreviousError counts when readable; only live windows have a report-traffic status and a separate three-valued lead for reports not marked earlier-session. `whea_reports` shows the separate Kernel-WHEA channel by report time, with its own coverage and PreviousError flags. The Hardware errors screen lets you filter returned report references by a trace stretch, then inspect a matching exact record in place; a report after restart is not dated as a new hardware error. |
