...
| Reading | What it is |
| --- | --- |
| `events`, `record` | Records from the System and Application logs by level, from a moment or Windows' reported kernel-session start, and the records *before* a moment; each reading shows how far back its log still reaches |
...
| `whea`, `whea_record`, `storms`, `whea_reports` | Hardware-error records from both Windows WHEA logs, with CPER header facts. `whea` gives up to 500 bounded newest-report previews with source coverage, a short message and fixed-header facts; `whea_record` retrieves one retained event by its source and log-local RecordId, including older reports, with full fields and System decoder detail. Exact binary bytes require an explicit unredacted request; default responses withhold them. Detailed decoding is currently limited to System WHEA-Logger records. `storms` counts that System source over live or historical filing-time buckets with coverage and fixed-header PreviousError counts when readable; only live windows have a report-traffic status and a separate three-valued lead for reports not marked earlier-session. Set `references=true` in a focused storm window to reach every returned System report by a bounded reference; default answers omit that extra list. `whea_reports` shows the separate Kernel-WHEA channel by report time, with its own coverage and PreviousError flags. The Hardware errors screen lets you filter returned report references by a trace stretch, then inspect a matching exact record in place; a report after restart is not dated as a new hardware error. |
