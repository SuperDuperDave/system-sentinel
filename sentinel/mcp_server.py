...
INSTRUCTIONS = (
    ...
    "An empty crash answer can still warn about an unplaced shutdown marker; inspect its warnings and raw rows before calling the window clean. "
    ...
    "When a Signals lead carries refs, call each ref's reading with its params; 'event_record' checks log, record_id and time_created. "
    "reference.status=same confirms that the re-read row has the cited time. Check refs_missing and refs_omitted. "
    "For a lead without refs, take its cited reading with the scope in method.readings[].params; "
    "a new take is a new observation, so compare asked_at. An aggregate sample cannot be reconstructed from a fresh read. "
    "Before stating a cause, inspect raw evidence. "
