...
INSTRUCTIONS = (
    ...
    "and that Signals does not classify hardware error reports. "
    "For a crash lead's refs, call each ref's reading (now 'event_record') with its params; it checks log, record_id and time_created. "
    "reference.status=same confirms that the re-read row has the cited time. Check refs_missing and refs_omitted. "
    "Other Signals leads have no exact-row refs: take their cited reading with the scope in method.readings[].params; "
